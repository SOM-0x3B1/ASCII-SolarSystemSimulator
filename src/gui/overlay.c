...
static void overlay_renderFooter(Program *program, Screen *screen, LayerInstances *li, Simulation *sim) {
    ...
    drawing_drawText(&li->overlayLayer, screen->width / 2 - (int) strlen((char *) sCamPos) / 2, screen->height - 2,
                     (char *) sCamPos, screen);

    char *sFollowing[32];
    sprintf((char *) sFollowing, "Following: %s", (sim->following == NULL) ? "[FREE]" : sim->following->name);
    drawing_drawText(&li->overlayLayer, screen->width - (int) strlen((char *) sFollowing) - 2, screen->height - 2,
